...
- **AI 助手**：独立视图，可在首页通过顶部 tab、左右方向键或左右滑动切换进入；用户可自行配置 OpenAI 兼容的 API 地址、API Key 与模型，支持常规问答、命名简称、报错解释、Commit Message、Regex、SQL 和 API 结构分析，AI 回复支持 Markdown 渲染与流式输出。不同模式使用独立对话记录，可单独清除当前模式记录。API Key 使用 WebCrypto 加密保存，设置后不回显，后续只能重新输入覆盖。
